...
- Cases batch Gate 1: approved on 2026-09-23. The approved existing-file-to-module map is recorded in `docs/gate-cases-mapping.md`; static dashboard/modal HTML and the shared CSS cascade remain in `ops/index.html`, while the selected case functions move verbatim to one classic-script module.
- Cases: complete on branch `codex/ops-cases-modularize` in the single slice commit named `refactor(ops): extract cases module`. Entry file is `ops/js/modules/cases.js`; it is loaded after `expenses.js` and before the remaining legacy inline script. The global names used by core UI, payables, expenses, attendance, profit and profit-share code remain unchanged.
- Cases validation evidence:
  - `node scripts/verify-ops-modularization.mjs`: 9 local scripts have valid JavaScript syntax; 8 extracted modules/blocks and 334 function bodies match their approved baselines byte-for-byte; `cases.js` contains the 34 approved case functions. The verifier uses `754460e32950a30e71de48f08c0e247cee99abb3` as the cases-batch baseline and protects `renderCurrentPage`, `refreshAccountingLinkedViews`, `applyRole`, and `submitNewClient` from incidental changes.
  - `firebase-database.rules.json` parses as JSON; `git diff --check` passes. `form/`, Firebase rules, `ops/js/core/data.js`, and the already accepted receivable/payable/expense modules have zero diff from `754460e`.
  - Local HTTP smoke returned 200 for `ops/` and all 9 local scripts, including `ops/js/modules/cases.js`. Browser smoke confirmed all 9 scripts loaded, `cases.js` rendered the case table and dashboard KPIs, all 18 page containers rendered or remained available for their existing lazy renderer, the approved case DOM bindings/modals remained present, and there were zero console warnings/errors.
  - Local HTTP intentionally displayed the login screen because only `file://` bypasses authentication; the controlled browser disallowed opening a `file://` URL. Therefore authenticated write flows and modal submissions were not exercised on the modular branch. This does not affect the byte-for-byte function-body evidence, and no attempt was made to bypass authentication or deploy a preview.
  - Production Firebase verification was read-only before and after the slice. The cloud snapshot remained at `2026/09/23 11:36`: CASES 27 (進行中 15 / 完工待結算 2 / 結案 10; contract amount $54,305,291); PAYABLES 572 (pending 1 / $120,000; approved 2 / $17,296; paid 569 / $43,860,677.25); RECEIVABLES 74 (collected 69 / $53,280,699; pending 5 / $1,572,421; missing invoice 1); EXPENSES 836 (pending $123; approved $1,793,709; total $1,793,832). No production create/update/delete/approval or other write action was performed, and the production console remained free of warnings/errors.
- Cases next step: present the complete diff and the evidence above for Gate 2 approval, then stop. Do not merge to `main`, push `main`, or deploy GitHub Pages before later explicit approval and a fresh Gate 3 production backup/comparison.
